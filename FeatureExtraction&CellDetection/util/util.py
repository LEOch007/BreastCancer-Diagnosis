...
def center_point(mask):
	...
	for cordinates in prop:
		...
		if not math.isnan(temp_center[0]) and not math.isnan(temp_center[1]):
			...
			center_points.append((int(temp_center[0]), int(temp_center[1])))
			center_mask+=dilation(temp_mask, square(2))
	return np.clip(center_mask, a_min=0, a_max=1).astype(np.uint8), center_points
...
def draw_individual_edge(mask):
	...
	for index in np.unique(individual_mask):
		...
		edge+=temp_edge
	return np.clip(edge, a_min=0, a_max=1).astype(np.uint8),individual_mask

def center_edge(mask, image):
	center_map, center_points=center_point(mask)
	edge_map, edge_points=draw_individual_edge(mask)
	...
	return check_image.astype(np.uint8), comb_mask.astype(np.uint8), center_points, edge_points